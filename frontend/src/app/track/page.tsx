      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6">
        ...
        {trackData && (
          <div className="glass-strong rounded-3xl p-8 lg:p-12 overflow-hidden relative">
                  ...
                  {trackData.orderItems.map((item: any, idx: number) => {
                    ...
                    return (
                      <div key={idx} className="flex justify-between text-sm">
                        <span className="text-white/70">{snapshot.name || 'Product'} ({snapshot.size}/{snapshot.color}) x {item.quantity}</span>
                        <span className="text-white">₹{(price * item.quantity).toLocaleString()}</span>
                      </div>
                    );
                  })}
                </div>
            ...
            {/* Shipping Address */}
            {(trackData.shippingAddressSnapshot || trackData.shippingAddress) && (
              <div className="pt-6 mt-6 border-t border-white/10">
                <h3 className="text-sm font-semibold text-white mb-2 flex items-center gap-2">
                  <Package className="w-4 h-4 text-accent" />
                  Shipping Address
                </h3>
                {(() => { const addr = trackData.shippingAddressSnapshot || trackData.shippingAddress; return (
