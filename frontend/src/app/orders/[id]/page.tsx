...
const statusConfig: Record<string, { icon: typeof Clock; color: string; label: string }> = {
  ...
  refunded: { icon: XCircle, color: 'text-orange-400', label: 'Refunded' },
};
...
export default function OrderDetailPage() {
  ...
  let dateToCheck = order.deliveryDateFormatted || order.estimatedDelivery;
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            {addr && (
              <div className="p-6 rounded-2xl glass-strong">
                ...
                <p className="text-sm text-white/50">{addr.addressLine1}{addr.addressLine2 ? `, ${addr.addressLine2}` : ''}</p>
            ...
            <div className="p-6 rounded-2xl glass-strong">
              ...
              <div className="space-y-2 text-sm">
                ...
                <p className="text-xs text-white/30 flex items-center gap-1">
                  <span className={`w-1.5 h-1.5 rounded-full ${order.paymentStatus === 'completed' ? 'bg-emerald-400' : 'bg-yellow-400'}`} />
