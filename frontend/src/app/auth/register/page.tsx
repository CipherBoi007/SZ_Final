...
import { Mail, Lock, Eye, EyeOff, User, Phone as PhoneIcon, ArrowRight, ArrowLeft, CheckCircle, XCircle } from 'lucide-react';
...
export default function RegisterPage() {
  ...
  const isOTPEnabled = process.env.NEXT_PUBLIC_OTP_VERI === 'ON';
  ...
  const handleSendOTP = async () => {
    ...
    setLoading(true);
    try {
      await authAPI.sendSignupOTP({ email, phone });
      setOtpSent(true);
      setShowOTP(true);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const { data } = await authAPI.register({ name, email, password, phone, otp: isOTPEnabled ? otp : undefined } as any);
  ...
  const handleGoogleLogin = () => {
    window.location.href = `${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/auth/google?redirect=${encodeURIComponent(window.location.origin)}`;
  };
    ...
    <div className="min-h-screen flex">
      ...
      <div className="flex-1 flex items-center justify-center p-6 pt-28 lg:p-16 lg:pt-32 bg-background">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }} className="w-full max-w-md">
          <Link href="/shop" className="inline-flex items-center gap-2 text-xs text-white/30 hover:text-white transition-colors mb-8 group">
          ...
          <form onSubmit={handleSubmit} className="mt-8 space-y-4">
            ...
            {showOTP && (
              <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} className="space-y-2">
                <label className="text-xs font-medium text-accent mb-1.5 block">Enter Verification OTP</label>
                <div className="relative">
                  <CheckCircle className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-accent/50" />
                  <input 
                    type="text" 
                    value={otp} 
                    ...
                    required 
                    placeholder="000000" 
                    className="w-full py-3.5 pl-11 pr-4 rounded-xl bg-accent/5 border border-accent/20 text-sm text-white placeholder:text-white/10 outline-none focus:border-accent transition-all font-mono tracking-[0.3em]" 
                  />
                </div>
                <p className="text-[10px] text-white/30 px-1">We've sent a 6-digit code to verify your account.</p>
              </motion.div>
            )}
            ...
            <button type="submit" disabled={loading} className="w-full flex items-center justify-center gap-2 py-4 rounded-xl bg-accent hover:bg-accent-hover text-white font-semibold transition-all glow-red-hover disabled:opacity-50 mt-6">
              {loading ? <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : 
                <>
                  {isOTPEnabled && !otpSent ? 'Send Verification OTP' : (otpSent ? 'Verify & Create Account' : 'Create Account')}
